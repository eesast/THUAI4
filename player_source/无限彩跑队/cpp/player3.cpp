...
std::default_random_engine r((unsigned)time(NULL) * 3);
bool g_coloring_3(GameApi& g);
...
void AI::play(GameApi& g)
{
    ...
    if (self->bulletNum == 0 && g.GetCellColor(self->x / 1000, self->y / 1000) != selfcolor && (clock() > askhelpclock + 5 * CLOCKS_PER_SEC))
    {
...
bool g_coloring_3(GameApi& g)
{
    ...
    colorcount++;
    int p = 3 * ((colorcount * 25 % 72) / 8) + 2;
    int q = 3 * ((colorcount * 25 % 72) % 8) + 26;
