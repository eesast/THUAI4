...
#define hisplace 30500
#define myplace 19500
	...
	if (g.TryGetMessage(msg))
	{
		...
		if (msgx != 55)
		{
			if (self->bulletNum > 1)
			{
				if (shot >= 2)
				{
					HappyAttack(g, self, (retranX(msg) + 3 * bulletposx) * 1000, (retranY(msg) + 3 * bulletposy) * 1000);
					bulletposx++;
					bulletposy--;
					if (bulletposx > 2)
						bulletposx = -2;
		...
		else if (msgx == 55)
		{
			...
			else if(self->y >= myplace && g.GetCellColor(self->x / 1000, self->y / 1000) != g.GetSelfTeamColor() && self->bulletNum == 0)
			{
				...
				g.Send(0, helpmsg);   //ID=1的角色只向ID=0的求助。
			}
		}
	}
