...
void bornplace_hl(GameApi& g) {
	...
	if ((state == -1 || state == 0 || state == 1 || state == 2 || state == 7 || state == 8))//not arriving at the top
	{
		if (self->x > 1500) {
			flag = 1;
		}
		else if (self->y <= 47500 && g.GetCellColor((self->x) / 1000, (self->y) / 1000 + 1) != g.GetSelfTeamColor()) {
			state = 0;
		}
		else if (self->y >= 2500 && g.GetCellColor((self->x) / 1000, (self->y) / 1000 - 1) != g.GetSelfTeamColor()) {
			state = -1;
		}
	}
